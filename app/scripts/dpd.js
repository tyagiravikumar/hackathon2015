/**
 * Created by RTYAGI on 1/09/2015.
 */
/** Dependencies **/
...
dpd.employee = dpd("/employee");
dpd.employee.on = function(ev, fn) {
  return dpd.on("employee" + ":" + ev, fn);
}
dpd.employee.rename = function (path, body, fn) {
  dpd.exec("rename", path, body, fn);
}

dpd.niitresourses = dpd("/niitresourses");
dpd.niitresourses.on = function(ev, fn) {
  return dpd.on("niitresourses" + ":" + ev, fn);
}
dpd.niitresourses.rename = function (path, body, fn) {
  dpd.exec("rename", path, body, fn);
}
